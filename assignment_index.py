...
def parser(title,id,text):
	...
	for i in dict_links:
		try:
			dict_all[i]+='e'+str(dict_links[i])
		except:
			dict_all[i]='e'+str(dict_links[i])
	for i in dict_category:
		try:
			dict_all[i]+='c'+str(dict_category[i])
		except:
			dict_all[i]='c'+str(dict_category[i])
	for i in dict_all:
		try:
			dict_posting[i]+='|'+str(id-min_id)+dict_all[i]
#                        print min_id
		except:
			dict_posting[i]=str(id-min_id)+dict_all[i]
	if sys.getsizeof(dict_posting)>10000000:###10 mb = 10000000 b
...
class ABContentHandler(xml.sax.ContentHandler):
	...
	def endElement(self, name):
		...
		if name=="title":
			...
			if min_id==None:
				min_id=int(self.id)
			f1=open('./src/Titles.txt','a')
			f1.write((self.id+' '+self.title+'\n').encode('utf-8'))
			f1.close()
...
if __name__ == "__main__":
	global stopwords,dict_posting,min_id,files_count,lines_count,documents
	documents=0
	files_count=1
	dict_posting={}
	stopwords={}
	min_id=None
	f1=open('./src/Titles.txt','w+')
	f1.close()
